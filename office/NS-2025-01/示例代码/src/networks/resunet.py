...
class SharedBottleNeck(nn.Module):
    def __init__(self, in_channels=512, depth=5, shared_depth=2, start_filters=32, blocks=1, residual=True,
        ...
        max_filters = 512
        for i in range(start_depth, depth): # i = 3, 4
    ...
    def forward(self, input):
        ...
        for i, d_conv in enumerate(self.down_convs):
            # d_conv, attn = nets
            x, before_pool = d_conv(x)
            # print(f"encoder convs  i:{i}, x.shape: {x.shape}, before_pool.shape: {before_pool.shape}")
        ...
        for i, nets in enumerate(zip(self.up_convs, self.up_im_atts)):
            ...
            # print(f"decoder convs  i:{i}, x.shape:{x.shape}")
        x_im = x
        ...
        for i, nets in enumerate(zip(self.up_convs, self.up_mask_atts)):
            ...
            # print(f"x.shape:{x.shape}, before_pool.shape:{before_pool.shape}")
            x = up_conv(x, before_pool, se = attn)
        x_mask = x
...
class CoarseDecoder(nn.Module):
    def __init__(self, args, in_channels=256, out_channels=3, norm='bn',act=F.relu, depth=5, blocks=1, residual=True,
    ...
    def forward(self, bg, fg, mask, encoder_outs=None):
        ...
        for i, up_convs in enumerate(zip(self.up_convs_bg, self.up_convs_mask)):
            ...
            # print(f"feats.shape: {mask_x.shape}, primary_map.shape: {primary_map.shape}, self_calibrated_map.shape{self_calibrated_map.shape}")
...
class SLBR(nn.Module):
    ...
    def __init__(self, args, in_channels=3, depth=5, shared_depth=2, blocks=3,
    ...
    def forward(self, synthesized):
        ...
        reconstructed_wm = wm
        # print(f"output mask.len: {len(mask)}")
        if self.refinement is not None:
            dec_feats = (ims)[1:][::-1]
